...
$agentPath = Join-Path $appDir "sync_agent.py"
$configPath = Join-Path $appDir "config.json"
$runnerPath = Join-Path $appDir "run-sync.ps1"
$taskName = "CompanyMonitorSync"

New-Item -ItemType Directory -Force -Path $appDir | Out-Null
Copy-Item -Force (Join-Path $scriptDir "sync_agent.py") $agentPath
...
`$ErrorActionPreference = 'Stop'
Set-Location '$appDir'
Invoke-Expression "& $PythonCommand '$agentPath' --config '$configPath'"
"@
